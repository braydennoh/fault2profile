...
where \(U\) is rock uplift, \(K\) is an erodibility coefficient, \(A\) is drainage area, and \(S = \partial z / \partial s\) is channel slope along streamwise coordinate \(s\). At steady state,
...
The misfit minimized in the MCMC/DE stage is
...
where weights \(w_i\) reflect node spacing along the channel.

In summary, the stream power equation enters the inversion as a **forward steady-state operator** mapping uplift (predicted from fault geometry) to river profile shape, while erodibility \(K\) is solved analytically for each trial geometry.
